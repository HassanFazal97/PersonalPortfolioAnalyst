...
1. ~~Ship billing live~~ **Done** — Railway prod has all three `billing_enabled()` vars (`STRIPE_SECRET_KEY`, `STRIPE_PRICE_PRO_MONTHLY`, `STRIPE_PRICE_PRO_ANNUAL`) plus `PUBLIC_BASE_URL` and `STRIPE_WEBHOOK_SECRET`, confirmed via `railway variables`; CAD price IDs match the $20/mo, $160/yr prices. "Coming soon" should no longer render on `/pricing`.
...
3. **Fix trust contradictions** — ~~contact email~~ done (`hello@cirvia.ca`). ~~Receiving~~ **Done, verified Aug 13**: ImprovMX forwards on the GoDaddy-hosted DNS with a catch-all alias (`*` → operator Gmail), so every address at the domain lands; MX/SPF/DMARC all return `valid: true` and a test send to `x@cirvia.ca` logged `DELIVERED`. Resend's outbound stays isolated on `send.cirvia.ca`, so the two don't collide. *Two known traps:* `improvmx domain check` v1.0.0 misrenders valid records as `MISSING` in its table — trust `--json`, not the table; and the catch-all forwards mail to any invented address, so swap it for explicit aliases if spam picks up. Inbound is forward-only (no SMTP send-as without ImprovMX Premium), so support replies still come from the operator's Gmail. ~~Legal-entity footer~~ **Done Aug 8, name removed Aug 12**: operator is an unregistered sole proprietorship, Ontario — described (without naming the individual) in the footer, Privacy §1, and the Terms opening paragraph. **Flagged, not resolved:** operating commercially under "Cirvia" without registering it as a business name may require an Ontario Business Names Act registration (Master Business Licence) — a business decision for the operator, not a code change.
...
5. ~~Canadian benchmarks + real risk-free~~ **Done Jul 30** — portfolio analytics benchmark is a geography-blended SPY(CAD)+XIC.TO total-return composite weighted by the book's own CAD/USD split, labeled in payloads; BoC Valet 3-month T-bill persisted nightly under a `daily_prices` pseudo-ticker, read (never fetched) at request time, `RISK_FREE_RATE_ANNUAL` override. Still open: the picks track record itself still benchmarks SPY alone — blend it by universe weights alongside the factor work (#6).
