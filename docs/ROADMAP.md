...
3. **Fix trust contradictions** — ~~contact email~~ done (`hello@cirvia.ca`; receiving setup is still a deploy blocker — DNS is at the registrar, not Cloudflare, so use a registrar-agnostic forwarder like ImprovMX). ~~Legal-entity footer~~ **Done Aug 8**: operator is Fazal Hassan, an unregistered sole proprietorship, Ontario — named in the footer, Privacy §1, and the Terms opening paragraph. **Flagged, not resolved:** operating commercially under "Cirvia" without registering it as a business name may require an Ontario Business Names Act registration (Master Business Licence) — a business decision for the operator, not a code change.
