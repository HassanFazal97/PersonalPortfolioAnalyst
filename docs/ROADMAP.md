...
4. **Trial mechanics** (`app/plans.py`) — start the 7-day clock at portfolio connect, not signup; on lapse, auto-downgrade to Free after 7 days of silence instead of pausing digests indefinitely (a Free user getting Monday digests is a live prospect; a paused user is churn). Trial-end notifications: T-2 days, day-of, +3.
5. **Reprice in CAD** — ~$19–21 CAD/mo, ~$160 CAD/yr via Stripe multi-currency. USD pricing to a Canadian audience contradicts the positioning. Grandfather existing USD subscribers (Stripe cannot convert a live subscription's currency in place — new price IDs, migrate on renewal or leave grandfathered).
6. **SnapTrade return-flow polling** — replace the manual "I've finished connecting" button with status polling (narrowest point of the funnel).
7. **Funnel instrumentation** (S) — the plan gates decisions on measurements (signup → connect → first-briefing funnel, the ~50-paying-users quant gate), so event tracking must be a build item, not an aspiration. Extend the existing `cirvia.funnel` structured-log pattern into server-side events persisted to Postgres (same house style as `agent_runs` observability); no client-side analytics needed to start.
